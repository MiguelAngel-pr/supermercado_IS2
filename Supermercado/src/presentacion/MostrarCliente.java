...
public class MostrarCliente extends JFrame implements IGUI {
    ...
    public MostrarCliente() {
        ...
        btnBuscar.addActionListener(e -> {
            ...
            try {
                if (selected == 0) {
                    Controller.getInstance().handleAction(Events.MOSTRAR_UNO_ID_CLIENTE, Integer.parseInt(valor));
                } else if (selected == 1) {
                    Controller.getInstance().handleAction(Events.MOSTRAR_UNO_NIF_CLIENTE, valor);
                } else {
                    Controller.getInstance().handleAction(Events.MOSTRAR_TODO_CLIENTE, null);
                }
                dispose();
