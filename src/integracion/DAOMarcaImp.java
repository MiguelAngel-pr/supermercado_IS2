...
public class DAOMarcaImp implements DAOMarca {
    ...
    public TMarca read(int id) {
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL)) {
            ...
            if (rs.next()) {
                tMarca = new TMarca(rs.getInt("id"), rs.getString("nombre"));
            }
            rs.close(); ps.close();
        } catch (SQLException e) { e.printStackTrace(System.err); }
        return tMarca;
    ...
    public Set<TMarca> readAll() {
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL)) {
            ...
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                set.add(new TMarca(rs.getInt("id"), rs.getString("nombre")));
            }
            rs.close(); ps.close();
        } catch (SQLException e) { e.printStackTrace(System.err); }
        return set;
    ...
    public int delete(int id) {
        int result = 0;
        try (Connection conn = DriverManager.getConnection(DB_URL)) {
            PreparedStatement ps = conn.prepareStatement("DELETE FROM Marca WHERE id=?");
            ps.setInt(1, id);
